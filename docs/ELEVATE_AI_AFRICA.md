...
| **"Inclusive, context-relevant, online and offline-first"**                           | USSD `*123*456#` flow on any feature phone (Telkom HMAC-signed), WhatsApp Cloud API channel, 15 SADC + African languages, WCAG 2.1 AA primitives, IndexedDB queue for offline reports. See `server/ussd/`, `server/routes/whatsappRoutes.ts`, `src/lib/i18n/`, `src/hooks/useOfflineSync.ts`.                      |
