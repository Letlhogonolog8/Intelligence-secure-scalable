...
import {
  SupabaseTimeouts,
  withTimeout as withSupabaseTimeout,
...
async function initializeServices() {
  ...
  const instanceUrls =
    process.env.INSTANCE_URLS?.split(",").filter(Boolean) || [];
...
function validateEnvironment(): void {
  ...
  const productionOnlyVars = ["TELKOM_WEBHOOK_SECRET"];
...
try {
  ...
  try {
    ...
    if (process.env.NODE_ENV === "production") {
      logger.error(
        "Cannot start without proper encryption in production",
        encryptionError,
      );
      process.exit(1);
    }
...
async function ensureNotificationQueueAvailable(): Promise<boolean> {
  try {
    const { error } = await withTimeout(
      (async () =>
        await supabase.from("notification_queue").select("id").limit(1))(),
      5000,
...
async function runNotificationWorkerCycle(): Promise<void> {
  ...
  try {
    const queueAvailable =
      twilioNotificationService.isQueueAvailable() ||
      (await ensureNotificationQueueAvailable());
    ...
    const processed =
      await twilioNotificationService.processPendingNotifications(25);
...
async function runAuditChainCheck(): Promise<void> {
  try {
    ...
    const result = await auditLogService.verifyChainDetailed();
    const duration = Date.now() - start;

    switch (result.status) {
      case "valid":
        logger.info("Audit chain verification succeeded", {
          durationMs: duration,
          verified: result.verified,
        });
        break;
      case "skipped":
        // Operational warning, NOT a security incident: the immutable
        // chain table is not deployed in this environment yet.
        logger.warn("Audit chain verification skipped", {
          durationMs: duration,
...
function startNotificationWorker(): void {
  ...
  const intervalMs = Math.max(
    5000,
    Number(process.env.NOTIFICATION_WORKER_INTERVAL_MS || 15000),
  );
...
function buildConnectSrcAllowlist(): string[] {
  ...
  if (
    ...
    process.env.VITE_DATADOG_ENABLED === "true"
  ) {
    list.push(
...
app.use((req: Request, res: Response, next: NextFunction) => {
  ...
  res.on("finish", () => {
    ...
    errorTracking.captureApiRequest(
      req.method,
      req.path,
      res.statusCode,
      duration,
...
async function requireAuth(
  ...
  try {
    ...
    if (!token) {
      res
        .status(401)
        .json({ error: "Authorization token required", requestId });
      return;
    }
...
app.post(
  ...
  requireAuth,
  mfaLimiter,
  validationMiddleware.mfaSetup,
  async (req: Request, res: Response): Promise<void> => {
    try {
      const userId = (req as AppRequest).user!.id;
      const { username } = req.body;
      const setup = await mfaService.generateMFASetup(
        userId,
        username || userId,
      );
      res.json({ setup });
...
app.post(
  ...
  requireAuth,
  mfaLimiter,
  validationMiddleware.mfaVerify,
...
app.post(
  ...
  requireAuth,
  escalationLimiter,
  idempotency({ scope: "escalate", ttlSeconds: 24 * 60 * 60 }),
  validationMiddleware.escalate,
  async (req: Request, res: Response) => {
    try {
      const requestId = (req as AppRequest).id;
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const { data, error } = await supabase
        .from("alerts_feed")
        .select(
          "id,time,type,message,module,status,created_at,acknowledged_at,acknowledged_by",
        )
      ...
      res
        .status(500)
        .json({ error: "Failed to retrieve police alerts", requestId });
    }
  },
);

app.post(
  "/api/police/alerts/:alertId/acknowledge",
  requireAuth,
  requirePoliceAccess,
  async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(500)
        .json({ error: "Failed to acknowledge police alert", requestId });
    }
  },
);
...
app.use(
  "/api/ussd",
  ussdJsonParser,
  createUSSDRoutes(ussdGateway, offlineCache),
);
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      if (!messages || !Array.isArray(messages) || messages.length === 0) {
        res
          .status(400)
          .json({ error: "messages array is required", requestId });
        return;
      }
      ...
      const craftFallback = () =>
        /danger|hurt|kill|die|attack|emergency/i.test(lastUser)
          ? "CRISIS ALERT: Please call Police: 10111 or Crisis Line: 0800 428 428 immediately. Dial *384*30933# from any phone — no internet needed. You are not alone."
...
4. If the user describes immediate danger, active violence, or suicidal ideation, respond with CRISIS ALERT: and provide emergency numbers (Police: 10111, Crisis line: 0800 428 428, USSD: *384*30933#)
...
app.post(
  "/api/ussd/telkom/callback",
  validationMiddleware.ussdCallback,
  async (req: Request, res: Response) => {
    try {
      const requestId = (req as AppRequest).id;
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!phoneNumber || userInput === undefined) {
        return res
          .status(400)
          .json({ error: "Missing phoneNumber or userInput", requestId });
      }

      const response = await ussdGateway.handleUSSDRequest(
        phoneNumber,
        userInput,
        language,
        sessionId,
    ...
    server.listen(Number(PORT), HOST, () => {
      ...
      logger.info("AEGIS-AI server startup", {
        ...
        features: {
          ...
          notificationWorker: isNotificationWorkerEnabled()
            ? "In-process"
            : "External/disabled",
        },
      });
...
const gracefulShutdown = async (signal: string) => {
  ...
  setTimeout(() => {
    logger.error(
      "Forced shutdown after 30 seconds timeout",
