...
import {
  ...
  saveSession,
  type WASession,
} from "../utils/whatsappSessionStore";
...
function detectLanguage(text: string): string {
  ...
  if (/\b(sawubona|ngicela|usizo|bika|siyabonga|yebo|ngifuna)\b/.test(l))
    return "zu";
...
async function getAIResponse(
  ...
  // Always return a string (never `null as unknown as string`) so callers
  // can rely on the type and use truthy checks for "AI unavailable".
  if (!apiKey) return "";
...
async function processMessage(phone: string, text: string): Promise<string> {
  ...
  const session = await getOrCreateSession(phone, lang);
  ...
  try {
    ...
    if (session.tier === "report") {
      ...
      const risk = /CRITICAL/i.test(aiSummary)
        ? "critical"
        : /HIGH/i.test(aiSummary)
          ? "high"
          : "medium";
      const riskEmoji =
