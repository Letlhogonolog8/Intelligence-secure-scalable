...
export function createIntelligenceRoutes(
  ...
  router.post('/risk-score', requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      const assessment = await riskEngine.assessCaseRisk(caseId, fullCaseData);

      // Store assessment in database
      const { error: storeError } = await supabase.from('ai_risk_scores').insert({
        case_id: caseId,
      ...
      // Broadcast assessment to relevant users via WebSocket
      // (integration with wsManager would happen here)
   ...
   */
  router.get('/risk-score/:caseId', requireAuth, async (req: Request, res: Response) => {
    try {
  ...
  router.post('/geo-match', requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      const assignment = await geoEngine.assignResources(caseId, lat, lng, riskLevel, caseType);
   ...
   */
  router.get('/explainability/:caseId', requireAuth, async (req: Request, res: Response) => {
    try {
      const { caseId } = req.params;
  ...
  router.get('/bias-audit', requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      const biasAudit = {
        ...
        biasDetectedCount: assessments?.filter((a) => a.bias_detected).length || 0,
        biasPercentage: assessments?.length
          ? (assessments.filter((a) => a.bias_detected).length / assessments.length) * 100
          : 0,
   ...
   */
  router.get('/threat-predictions', requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      res.status(500).json({ error: 'Failed to retrieve threat predictions' });
    }
  });
