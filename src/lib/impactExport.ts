...
export function buildPolicyBrief(
  ...
  const lines: string[] = [
    ...
    ``,
    `OUTCOMES`,
    `- Cases resolved: ${metrics.casesResolved.toLocaleString("en-US")}`,
    `- Survivors supported: ${metrics.survivorsSupported.toLocaleString("en-US")}`,
    `- Average response time: ${metrics.avgResponseMinutes} minutes`,
    `- Resources connected: ${metrics.resourcesConnected.toLocaleString("en-US")}`,
    ...
    ...provinces.map(
      (p) =>
        `- ${p.province}: ${p.cases.toLocaleString("en-US")} cases, ${p.coverage}% coverage`,
    ),
    ``,
