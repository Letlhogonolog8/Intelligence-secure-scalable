...
const sendPayload = (payload: LogPayload) => {
  ...
  if (typeof navigator !== "undefined" && "sendBeacon" in navigator) {
    navigator.sendBeacon(env.VITE_LOG_ENDPOINT, body)
    return
  }
  fetch(env.VITE_LOG_ENDPOINT, {
    method: "POST",
...
const sendDatadogPayload = (payload: LogPayload) => {
  if (!env.VITE_DATADOG_LOGS_ENDPOINT || !shouldSend()) {
    return
  }
  ...
  if (typeof navigator !== "undefined" && "sendBeacon" in navigator) {
    navigator.sendBeacon(env.VITE_DATADOG_LOGS_ENDPOINT, body)
    return
  }
  fetch(env.VITE_DATADOG_LOGS_ENDPOINT, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body,
    keepalive: true,
