  ...
  const handleFiles = useCallback(
    async (fileList: FileList | null) => {
      ...
      for (const file of toUpload) {
        ...
        const storagePath = `${user?.id ?? "anon"}/${fileId}/${file.name}`;
        ...
        try {
          ...
          // The `evidence` bucket is private; a signed URL is required to read it back
          // (getPublicUrl would return a URL that 400s, since the bucket has public: false).
          ...
          setFiles((prev) =>
            prev.map((f) =>
              f.id === fileId
                ? { ...f, status: "uploaded", url: urlData?.signedUrl }
                : f,
            ),
          );
        } catch {
              ...
              prev.map((f) =>
                f.id === fileId
                  ? {
                      ...f,
                      status: "pending",
                      localBlob: reader.result as string,
                    }
                  : f,
              ),
            );
      ...
      <div className="flex items-start gap-3 p-4 rounded-2xl bg-emerald-500/5 border border-emerald-500/15">
        ...
        <div>
          <p className="text-sm font-bold text-emerald-300">
            Anonymous Evidence Vault
          </p>
          <p className="text-xs text-slate-400 mt-0.5">
            Files are encrypted at rest and stored anonymously. Each upload
            generates a unique{" "}
            <strong className="text-slate-300">Access Code</strong> — keep it
            safe. No names, IDs, or location metadata are attached. POPIA
            compliant.
          </p>
        </div>
      </div>
          ...
          <div className="flex flex-col items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-1">
                Images · Video · Audio · PDF · Text &nbsp;·&nbsp; Max{" "}
                {MAX_FILE_MB}MB per file
              </p>
            </div>
            <Button
              size="sm"
                    ...
                    <div className="rounded-xl bg-black/30 border border-white/8 px-3 py-2.5 space-y-1.5">
                      <div className="flex items-center justify-between">
                        ...
                        <p className="text-[10px] text-slate-500">
                          Save this to retrieve your evidence
                        </p>
                      </div>
                      <div className="flex items-center gap-2">
                        <code
                          className={`flex-1 font-mono text-sm font-bold tracking-widest transition-all ${
                            revealed
                              ? "text-emerald-400"
