...
export const TrustedContactsCard: React.FC<TrustedContactsCardProps> = ({
  isLoading,
  headline,
  meta,
  actionLabel,
  ...
  <Card className="border-white/15 bg-slate-950/70 shadow-xl backdrop-blur-sm">
    <div className="p-5">
      <p className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-3">Trusted Contacts</p>
      {isLoading ? (
        <>
          <Skeleton className="h-7 w-28 bg-slate-800/60" />
          <Skeleton className="mt-2 h-3 w-40 bg-slate-800/60" />
          <Skeleton className="mt-4 h-9 w-32 bg-slate-800/60" />
        </>
      ) : (
        <>
