...
type SidebarRenderSection = Omit<SidebarSection, "modules"> & {
...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const favoriteModsData = visibleModules.filter((m) =>
    favoriteModules.includes(m.id),
  );
  const recentModsData = visibleModules.filter(
  ...
  const renderRoleSpecificSections = useMemo(() => {
    ...
    return roleConfig.sections.reduce<SidebarRenderSection[]>(
      (sections, section: SidebarSection) => {
        const sectionModules = visibleModules.filter(
          (m) =>
            section.modules.includes(m.id) && !highlightedModuleIds.has(m.id),
        );
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case "updating":
        return (
          <ActivityIcon size={10} className="text-blue-400 animate-spin" />
        );
      case "loading":
        return (
          <ActivityIcon size={10} className="text-amber-400 animate-pulse" />
        );
      default:
  ...
  }> = ({ mod, isFavorite, showNotification }) => {
    ...
    return (
      ...
      >
        ...
        >
          ...
          {!collapsed && (
            <>
              ...
              <div className="flex items-center gap-2">
                {typeof showNotification === "number" &&
                  showNotification > 0 && (
                ...
                {isActive && (
                  <ChevronRightIcon className="ml-auto opacity-50" size={14} />
                )}
              </div>
            </>
          )}
          {collapsed && isActive && (
            <div
              className={`absolute left-full ml-2 px-2 py-1 bg-slate-800 rounded text-xs whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity z-50`}
            >
              {mod.label}
            </div>
          )}
  ...
  return (
    <div
      className={`${collapsed ? "w-[72px]" : "w-64"} bg-slate-950 border-r border-slate-800/50 flex flex-col transition-all duration-300 relative z-20 overflow-visible`}
    >
      {/* Logo */}
        ...
        <div className="flex items-center gap-3">
          ...
          {!collapsed && (
            <div className="overflow-hidden">
              ...
              <p className="text-[10px] text-cyan-400 font-medium tracking-widest">
          ...
          renderRoleSpecificSections.length > 0 ? (
            renderRoleSpecificSections.map((section) => (
              <div key={section.id}>
                ...
                <div
                  className={`space-y-1 ${!collapsed && section !== renderRoleSpecificSections[renderRoleSpecificSections.length - 1] ? "mb-3 pb-3 border-b border-slate-800/50" : ""}`}
                >
            ...
            <>
              ...
              <div className="space-y-1">
                {(sidebarSearchQuery ? filteredModules : visibleModules).map(
                  (mod) => (
                    <div key={mod.id} className="relative group">
                      <div className="flex items-center">
                        <ModuleButton
