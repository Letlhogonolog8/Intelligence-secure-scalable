...
const WEB_INITIALIZABLE_ROLES = new Set<UserRole>([
  "admin",
  "counselor",
  "survivor",
  "ngo",
...
const ProfileInitialization: React.FC<ProfileInitializationProps> = ({
  ...
  const activeRole =
    resolvedRole && policy && WEB_INITIALIZABLE_ROLES.has(resolvedRole)
      ? resolvedRole
      : null;
  const allowsSelfInitiatedAccess = activeRole
    ? canSelfRegister(activeRole) || canRequestPrivilegedAccess(activeRole)
    : false;
  ...
  const isPendingApprovalRequest = activeRole
    ? canRequestPrivilegedAccess(activeRole) && !isAdminCreator
    : false;
  ...
  const handleProfileCreated =
    onProfileCreated ??
    ((_profile: ProfileInitializationPayload) => {
      navigate("/auth");
    });
  ...
  const normalizeAuthMessage = (message: string) =>
    message.replace(/email/gi, "username");
  ...
  const handleDetectLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        setLocationStatus("error");
        setError(
          "Unable to detect GPS location. Please enter coordinates manually.",
        );
      },
    );
  };
  ...
  const handleNextStep = () => {
    ...
    if (currentStep === "personal") {
      ...
      if (!usernamePattern.test(systemAlias.trim())) {
        setError(
          "Username must be 3-24 characters and may include letters, numbers, dots, dashes, or underscores",
        );
        return;
      }
      if (!phoneNumber.trim()) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const approvalStatus = isPendingApprovalRequest
        ...
        : activeRole === "survivor"
          ...
          : requiresApproval
            ? "approved"
            : undefined;
      const isActive = isPendingApprovalRequest
        ? false
        : !requiresApproval || isAdminCreator;
      ...
      if (
        ...
        const detail =
          createData?.error || createError?.message || "Registration failed";
        const message =
          activeRole === "survivor" && createData?.user_id
  ...
  const roleInfo = activeRole
    ? roleSpecificFields[activeRole]
    : roleSpecificFields.survivor;
  ...
  const verificationNotes = [
    activeRole === "survivor"
      ? "Location and consent fields support survivor-safe routing."
      : "Professional details are reviewed before role activation.",
    policy?.requiresApproval
      ? "Approval workflow is enforced for this role."
      : "No manual approval required after successful setup.",
    policy?.requiresBiometric
      ? "Biometric enrollment is recommended for this role."
      : "Password-based access is the default sign-in path.",
  ];
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            >
              <p className="text-sm text-slate-300">
                Use the minimum personal and professional detail required for
                safe routing, verification, and access control.
              </p>
            </AuthInfoPanel>
          </div>
        </motion.div>
            ...
            <CardContent className="pt-6">
              ...
              >
                {currentStep === "personal" && (
                  <div className="space-y-4">
                    <div className="rounded-2xl border border-white/10 bg-white/5 p-4 text-sm text-slate-300">
                      Enter the minimum required identity details for this role.
                    ...
                    {activeRole === "survivor" ? (
                      <>
                        ...
                        <div className="space-y-2">
                          ...
                          <p className="text-xs text-slate-400">
                            Auto-detect with device GPS or enter manually.
                          </p>
                        </div>
                ...
                {currentStep === "professional" && (
                  <div className="space-y-4">
                    <div className="rounded-2xl border border-white/10 bg-white/5 p-4 text-sm text-slate-300">
                      Add the professional details needed to verify operational
                      access and organizational affiliation.
                    </div>
                    {activeRole === "survivor" ? (
                      <div className="space-y-2">
                        <Label className="text-slate-300">
                          Support Organization (Optional)
                        </Label>
                        <Input
                          type="text"
                      ...
                      <>
                        ...
                        {(activeRole === "police" || activeRole === "ngo") && (
                          <div className="space-y-2">
                            ...
                            <Input
                              ...
                              onChange={(e) =>
                                setLegalDesignation(e.target.value)
                              }
                              className="bg-slate-950/70 border-slate-800 text-white focus-visible:ring-2 focus-visible:ring-blue-500/40 focus-visible:border-blue-400/50"
                            />
                          </div>
                        ...
                        <Alert className="border-rose-500/40 bg-rose-500/10">
                          ...
                          <AlertDescription className="text-rose-200">
                            Your professional information will be verified
                ...
                {currentStep === "security" && (
                  <div className="space-y-4">
                    <div className="rounded-2xl border border-white/10 bg-white/5 p-4 text-sm text-slate-300">
                      Complete password setup and optional security factors
                      before submitting the profile.
                    </div>
                    <div className="space-y-2">
                      ...
                      <p className="text-xs text-slate-400">
                        Minimum 8 characters, include upper/lowercase, numbers,
                        and symbols
                      </p>
                    </div>
                      ...
                      <div className="space-y-2">
                        <Label className="flex items-center gap-2 text-slate-300">
                          <input
                            ...
                            onChange={(e) =>
                              setEnableBiometric(e.target.checked)
                            }
                            className="w-4 h-4 rounded bg-slate-800 border-slate-600"
                          />
                        ...
                        <p className="text-xs text-slate-400 ml-6">
                          Set up fingerprint or face recognition for faster
                          login (recommended for your role)
                        </p>
                      </div>
                    )}
