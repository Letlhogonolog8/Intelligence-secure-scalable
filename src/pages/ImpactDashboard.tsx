...
import {
  ...
  ArrowLeft, Globe, CheckCircle, AlertTriangle, Phone, RefreshCw,
} from "lucide-react";
...
const ImpactDashboard: React.FC = () => {
  ...
  const { data: metrics, isLoading: metricsLoading, refetch, dataUpdatedAt } = useImpactMetrics();
  const { data: provinceData, isLoading: provinceLoading } = useProvinceMetrics();
  ...
  const outcomes = metrics
    ? [
        { label: "Cases Resolved", value: formatNumber(metrics.casesResolved), icon: CheckCircle, color: "text-emerald-400", bg: "bg-emerald-500/10", border: "border-emerald-500/20" },
        { label: "Avg. Response Time", value: `${metrics.avgResponseMinutes} min`, icon: Clock, color: "text-blue-400", bg: "bg-blue-500/10", border: "border-blue-500/20" },
        { label: "Survivors Supported", value: formatNumber(metrics.survivorsSupported), icon: Heart, color: "text-rose-400", bg: "bg-rose-500/10", border: "border-rose-500/20" },
        { label: "Resources Connected", value: formatNumber(metrics.resourcesConnected), icon: Users, color: "text-purple-400", bg: "bg-purple-500/10", border: "border-purple-500/20" },
        { label: "Provinces Active", value: `${metrics.provincesActive} / 9`, icon: MapPin, color: "text-amber-400", bg: "bg-amber-500/10", border: "border-amber-500/20" },
        { label: "Countries", value: String(metrics.countriesActive), icon: Globe, color: "text-cyan-400", bg: "bg-cyan-500/10", border: "border-cyan-500/20" },
      ]
    : [];
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-500">
                    Data anonymised · Live{updatedLabel ? ` · Updated ${updatedLabel}` : ""} · POPIA compliant
                  </p>
                </div>
              </div>
              <button
        ...
        >
          {metricsLoading
            ? Array.from({ length: 6 }).map((_, i) => (
                <div key={i} className="p-5 rounded-2xl border border-white/10 bg-slate-950/60 animate-pulse h-24" />
              ))
            : outcomes.map((item) => (
                <div
                  key={item.label}
