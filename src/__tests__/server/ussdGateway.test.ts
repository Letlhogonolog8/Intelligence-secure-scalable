...
describe("USSDGateway", () => {
  ...
  describe("handleUSSDRequest", () => {
    ...
    it("should persist simulator-provided session ids when creating a new session", async () => {
      const response = await gateway.handleUSSDRequest(
        "27821234567",
        "",
        "en",
        "AT_SIM_123",
    ...
    it("should render multilingual main menu and prompts in Zulu", async () => {
      const mainResponse = await gateway.handleUSSDRequest(
        "27821234567",
        "",
        "zu",
      );
      ...
      const reportPrompt = await gateway.handleUSSDRequest(
        "27821234567",
        "1",
        "zu",
      );
    ...
    it("should return case status if case exists", async () => {
      ...
      // After answering, the session returns to the main menu so the next
      // keypress is a valid choice; the status line is shown above the menu.
      expect(response.menu).toBe("main");
      expect(response.text).toContain(
        "Case CASE123: investigating (Risk: high)",
      );
      expect(response.endSession).toBe(false);
    });
    ...
    it("should return not found if case does not exist", async () => {
      ...
      // Returns to the main menu with the "not found" message shown above it.
      expect(response.menu).toBe("main");
  ...
  describe("handleTelkomCallback", () => {
    it("should process webhook payload and call sendTelkomResponse", async () => {
      ...
      expect(gateway.sendTelkomResponse).toHaveBeenCalledWith(
        "27821234567",
        expect.any(String),
        false,
      );
